...
// ToDo
// bug: when stopping game with on/off button,
// game only turns back on after two clicks
...
let mainCounter= 0; // mainCounter will log the turns
	...
	// using the web-audio api (built into the browser)
	// assign a new instance of AudioContext
	// to play game sounds
	...
	function startWave(color) {
		...
		osc.type = 'square';
		osc.start(0);
		// play sound without volume control (100%)
		//osc.connect(context.destination);

		// assign a gain node to control volume
		let gainNode = context.createGain();
		// connect the source to the gain node.
		osc.connect(gainNode);
		// connect the gain node to the destination.
		gainNode.connect(context.destination);
		//console.log(context,osc);
		// control volume
		gainNode.gain.value = 0.1;

		// assign a reference to the oscillator so we
